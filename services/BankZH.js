...
/*                                b1 = contents.find('p[align]').eq(1),
                            ...
                            async.forEach(arry, (val, callback) => {
                                ...
                                if(src.indexOf("http")!=0){
                                    let in1=subUrl.lastIndexOf('/'),in2=src.indexOf('/')
                                    src=subUrl.substring(0,in1)+src.substring(in2)
                                }
            ...
            redis.getAsync(redisKey).then((ress) => {
                if (!ress || ress != '1') {
                    this.getResult(url59 + href.substr(2), (da2) => {
                        ...
                        let contents = _$('div.sub_con > p');                                       //减少长度
                        contents=_.rest(_.initial(_.initial(_.initial(_.initial(contents)))))           //减少长度
                        let articles = [];
                        let aStr = '';
